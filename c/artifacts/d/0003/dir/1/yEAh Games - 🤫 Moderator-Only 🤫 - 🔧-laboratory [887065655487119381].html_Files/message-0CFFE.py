...
async def on_message(message):
    ...
    elif "yeahtm3039210" in message.content.lower():
        await client.change_presence(activity=discord.Activity(
            type=discord.ActivityType.playing, name=" Testing — may not work"))
    ...
    if message.content.startswith('yeahtm3039210'):
        await message.channel.send('⚠️ yEAh Games - bot is now in its testing phase; some features **may not work correctly**. Please be patient until the bot comes back online. ⚠️')
    if message.content.startswith('yeahsecretpwHF93D93d9bdu28hD830Dunale2o\dsue9203 hj\hsbdeezntutsoe9w3oweo'):
        await message.channel.send('You are the master; you have entered the secret password! You will not be able to find the secret in the message, though, but if you do, something cool might happen.')
    ...
    if message.content.startswith('yeahbalance 637006296234721320'):
        embedVar = discord.Embed(
            title="Yank | yEAh Games ",
            description="<@637006296234721320>", color=0xff4747)
        embedVar.set_author(
            name="yEAh Games",
            url="https://discord.gg/GNnPQ2h5",
            icon_url=
            "https://media.discordapp.net/attachments/887065655487119381/895986163020627980/yeahgames.png?width=751&height=422")
        embedVar.add_field(name="Balance",
                           value="――――――――― \n ",
                           inline=False)
        embedVar.set_footer(text="Bot developed by Nnillat#2657")
        await message.channel.send(embed=embedVar)
    if message.content.startswith('yeahyank'):
        embedVar = discord.Embed(
            title="Yank | yEAh Games ",

            description="You can view the balance of any member by typing `yeahbalance <userid>`, and it will show their yank account balance; it will show their balance in online yollars, and their balance in paper money. It also shows the exact yenominations that they have. Exchanging paper money for online money, or vice versa, is available in <#910282955132395530>; please follow the format provided. \n\n Keeping money online is completley secure, no one can rob you while the money is online, and you can exchange online money to cash when you need it. It is a recommened choice as robbing members' money is legal.\n\n In order to convert cash to online money, you must return the yollars to the yank-keeper, ", color=0xff4747)
        embedVar.set_author(
            name="yEAh Games",
            url="https://discord.gg/GNnPQ2h5",
            icon_url=
